...
//
//  Scripted trailer: hand from offset start → bomber tile (12,2); orange outline + scroll to enemy zone;
//  bomber column at col 1 (3 sequential drops, middle hit) — tap anywhere to dismiss.
//

import SwiftUI
import WatchKit

struct BomberDemo: View {
    let onClose: () -> Void
    ...
    private static let bombDropSpacing: TimeInterval = 1
    ...
    private func runSequence(
        ...
        // Bomber column: drops from tapped tile (`Rules.bombingPositions`). Middle of three is hit; rest miss.
        let drops = Rules.bombingPositions(target: Self.enemyBomberTarget, attacker: .player)
        for (index, pos) in drops.enumerated() {
            let kind: ExplosionKind = (drops.count == 3 && index == 1) ? .hit : .miss
            var overlays = missileImpactOverlays
            overlays[pos] = kind
            let isLast = index == drops.count - 1
            withAnimation(.easeOut(duration: 0.2)) {
                missileImpactOverlays = overlays
                if isLast { showHitBanner = true }
            }
            if !isLast {
                try? await Task.sleep(for: .seconds(Self.bombDropSpacing))
            }
        }
    }
